...
*/
#define PROPERTIES_FILE_PATH "/home/dreamvu/catkin_ws/src/dreamvu_pal_navigation/src/SavedPalProperties.txt"
...
void publishLaser(cv::Mat scan_mat, timeval timestamp)
{
...
int main(int argc, char **argv)
{
	...
	leftpub = it.advertise("/dreamvu/pal/odoa/get/left", 1);
	laserPub = nh.advertise<sensor_msgs::LaserScan>("/dreamvu/pal/odoa/get/scan", 1);
	...
	while (g_bRosOK)
	{
		...
		int laserscanSubnumber = laserPub.getNumSubscribers();
		int subnumber = leftSubnumber + laserscanSubnumber;
		...
		if (subnumber > 0)
		{
			...
			overlaid = (leftSubnumber && laserscanSubnumber);       
		...
		if (leftSubnumber > 0)
		{
            publishimage(overlaid ? data.marked_left  : data.left, leftpub, "bgr8", timestamp);
        }

		if (laserscanSubnumber > 0)
		{
			publishLaser(data.scan, data.timestamp);
		}
